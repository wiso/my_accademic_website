...
function initScroll(){
	...
	links.each(function(){
		...
		link.bind('click',function(e){
			jQuery('html,body').animate({scrollTop:jQuery(href).offset().top - (win.width() > maxWidth ? nav.outerHeight() : 0)},{duration: animSpeed, queue: false});
			e.preventDefault();
		});
	});
};
	...
	jQuery.fn.fixedSlideBlock = function(options) {
		...
		return this.each(function(){
			...
			function recalculateDimensions() {
				...
				fixedBlock.css({position:'',left:'',top:'',bottom:''});
				d = {
			...
			function checkStaticPosition() {
				...
				if(newHeight < origHeight) {
					fixedBlock.css({position:'', top:'', left:'', bottom:''});
					return true;
				} else {
					fixedBlock.attr('style',origStyle);
				}
			...
			function positionFixed() {
				if(d.scrollTop > d.blockOffset.top - options.extraHeight && maxWidth < win.width()) {
					// check that block fits in holder
					if(d.scrollTop + options.extraHeight - d.holderOffset.top + d.blockHeight > d.holderHeight) {
						fixedBlock.css({position:'absolute', left: d.blockPosition.left, top: d.blockPosition.top + d.holderHeight - d.blockHeight - (d.blockOffset.top - d.holderOffset.top), bottom:'auto'});
					}
					// set default fixed position
					else {
						fixedBlock.css({position:'fixed', left: d.blockOffset.left - d.scrollLeft, top: options.extraHeight, bottom:'auto'});
					}
				} else {
					// disable sticking
					fixedBlock.css({position:'', top:'', left:'', bottom:''});
			...
			function positionAbsolute(noAnimation) {
				...
				fixedBlock.stop().css({position:'absolute', left: d.blockPosition.left, bottom:'auto'});
